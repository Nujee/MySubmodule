...
{
    public sealed class ShaderOccurenceWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Search"))
            {
                if (shaderName != null && shaderName.Length > 0)
                {
                    ...
                    for (int i = 0; i < allMaterials.Length; i++)
                    {
                        ...
                        {
                            if (allMaterials[i] != item && item.ToLower().Contains(shaderName.ToLower()) &&
                                !materials.Contains(allMaterials[i]))
                            {
                                Debug.Log(item);
                                materials.Add(allMaterials[i]);
                            }
            ...
            if (GUILayout.Button("Select All Materials"))
            {
                ...
                {
                    var materialsGO = new List<Object>();
                    foreach (var material in materials)
                    {
                        materialsGO.Add(AssetDatabase.LoadAssetAtPath(material, typeof(Material)));
                    }

                    Selection.objects = materialsGO.ToArray();
                }
            }
